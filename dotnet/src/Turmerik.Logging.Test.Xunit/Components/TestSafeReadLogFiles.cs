...
using Turmerik.Logging.Logger;

namespace Turmerik.Logging.Test.Xunit.Components
{
    public class TestSafeReadLogFiles
    {
        ...
        {
            using SharedFileLogger fileLogger = LoggerFactory.Instance.GetSharedFileLogger(GetType());
        ...
        {
            using VerboseFileLogger bulkFileLogger = LoggerFactory.Instance.GetVerboseFileLogger(GetType());
        ...
        {
            string loggerFilePath = GetLoggerFilePath(fileName);
